  ...
  def test_tp_review_with_project_specified
    with_temp_readymade_project do |dir|
      ...
      begin
        ...
        output = tp_review_with_fixture(dir, 'tp-review-2', %w(a r), false, project_default[:title])
        assert_equal(0, output[:status].to_i, "Bad exit code: #{output[:status]} err: #{output[:err]}")
        assert_equal(3, project.local.file('data','sandbox-assignment.csv').as(:csv).reject{|assignment| assignment['hit_id'].to_s.empty? }.count)
        reviews = split_reviews(output[:out])
   ...
   def test_tp_review_without_project_specified
     ...
     with_temp_readymade_project do |transcripts_dir|
      ...
      begin
        ...
        output = tp_review_with_fixture(transcripts_dir, 'tp-review-4', %w(a r a a), false)
        assert_equal(0, output[:status].to_i, "Bad exit code: #{output[:status]} err: #{output[:err]}")
        reviews = split_reviews(output[:out])
